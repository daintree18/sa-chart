...
>>>>>>> 9041755cfdf848bd091c42c1b08136e1acf58ba1
  	.directive('saChartThermostat', function ($rootScope, zoneStatusApi, Attribute, $filter, $timeout)
  	{
  		var directive =
  		{
<<<<<<< HEAD
  			templateUrl: 'bower_components/sa-chart/thermostat/saChartThermostat.html',
=======
  			templateUrl: 'components/saChart/thermostat/saChartThermostat.html',
>>>>>>> 9041755cfdf848bd091c42c1b08136e1acf58ba1
  			restrict: 'EA',
  			scope:
  			{
	  					...
	  					{		
	  						...
	  						zoneStatusApi.get(scope.data.facility.uuid, scope.data.zone.uuid, start, end)
			  					.then(function (response)
			  					{
			  						...
			  						{
			  							if (status.attribute === Attribute.TEMPERATURE_SETPOINT_COOLING)
			  							{
			  								datasets[0].data.push([status.timeref, status.valueF]);
			  							}
			  							else if (status.attribute === Attribute.TEMPERATURE_SETPOINT_HEATING)
			  							{
			  								datasets[1].data.push([status.timeref, status.valueF]);
			  							}
			  							else if (status.attribute === Attribute.TEMPERATURE_THERMOSTAT)
			  							{
			  								datasets[2].data.push([status.timeref, status.valueF]);
			  							}
			  						});
			  						
			  						// Watch for only a single set point
			  						if (datasets[0].data.length === 1)
			  						{
			  							var setpoint = datasets[0].data[0][1];
			  							datasets[0].data[0][0] = datasets[2].data[0][0];
			  							datasets[0].data.push([datasets[2].data[datasets[2].data.length - 1][0], setpoint]);
			  						}
			  						if (datasets[1].data.length === 1)
			  						{
			  							var setpoint = datasets[1].data[0][1];
			  							datasets[1].data[0][0] = datasets[2].data[0][0];
			  							datasets[1].data.push([datasets[2].data[datasets[2].data.length - 1][0], setpoint]);
			  						}
			  						...
			  						if (scope.$parent.isShowThermostat())
			  						{
